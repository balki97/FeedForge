...
def test_inspect_feedpak_warns_when_all_separated_stems_are_disabled(tmp_path):
    package = tmp_path / "song.feedpak"
    output = tmp_path / "edited.feedpak"
    guitar = tmp_path / "guitar.ogg"
    guitar.write_bytes(b"OggSguitar")
    ...
    feedpak.update_feedpak(
        package,
        output,
        stem_updates=[{"id": "guitar", "file": str(guitar), "default": False}],
    )

    preview = feedpak.inspect_feedpak(output)

    assert preview["stems"][1]["default"] is False
    assert preview["warnings"] == [
        "Separated stems are present but all are disabled by default. Reprocess this FeedPak to enable playback through the stem mixer."
    ]
