...
def test_auto_device_uses_cpu_when_cuda_cannot_execute(monkeypatch):
    class BrokenCuda:
        @staticmethod
        def is_available():
            return True
    ...
    fake_torch = SimpleNamespace(
        ...
        ones=lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("unsupported gpu")),
    )

    demucs_server._DEVICE_CACHE.clear()
    monkeypatch.setitem(__import__("sys").modules, "torch", fake_torch)

    assert demucs_server.resolve_device("auto") == "cpu"
