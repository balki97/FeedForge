...
def test_rs1_multisong_grouping_can_borrow_audio_from_songs_psarc():
    ...
    songs_content = {
        "audio/windows/song_afterlife.bnk": b"\x00" * 44 + (111111).to_bytes(4, "little"),
        "audio/windows/song_afterlife_preview.bnk": b"\x00" * 44 + (222222).to_bytes(4, "little"),
        "audio/windows/song_barracuda.bnk": b"\x00" * 44 + (333333).to_bytes(4, "little"),
    ...
    assert "audio/windows/song_afterlife_preview.bnk" not in afterlife
    assert "audio/windows/111111.wem" in afterlife
    assert "audio/windows/222222.wem" not in afterlife
...
def test_convert_psarc_can_package_demucs_stems(tmp_path, monkeypatch):
    ...
    assert manifest["stems"][2]["default"] is False
...
def test_complete_demucs_stems_are_default_mix(tmp_path, monkeypatch):
    ...
    assert all(by_id[stem]["default"] is False for stem in ["guitar", "bass", "drums", "vocals", "other"])
...
def test_stem_separation_keeps_full_mix_after_success(tmp_path, monkeypatch):
    ...
    assert stems[1]["default"] is False
    assert full.exists()
    assert warnings == []
